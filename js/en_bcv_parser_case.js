(function() {
  ...
  bcv_parser = (function() {
    ...
    bcv_parser.prototype.match_books = function(s) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        book = _ref[_i];
        s = s.replace(book.regexp, function(match) {
          var extra;
          books.push({
            value: match,
            parsed: book.osis
    ...
    bcv_parser.prototype.match_passages = function(s) {
      ...
      while (match = this.regexps.escaped_passage.exec(s)) {
        ...
        if (!/(?:\d[\s*]*[a-e]|\d\W*title|\d\W*f+)\s*$/i.test(part)) {
          re = /[^\df.\x1e\x1f\)\]]+$/;
          part = part.replace(re, "");
          full = full.replace(re, "");
        }
        if (this.options.captive_end_digits_strategy === "delete") {
          next_char = match.index + full.length;
    ...
    bcv_parser.prototype.starts_with_book = function(passage) {
      ...
      if (passage.type === "range" && passage.start.type.substr(0, 1) === "b") {
        return true;
      }
      return false;
    };
  ...
  bcv_passage = (function() {
    ...
    bcv_passage.prototype.range = function(passage, accum, context) {
      ...
      if ((_ref6 = passage.absolute_indices) == null) {
        passage.absolute_indices = this.get_absolute_indices([start.indices[0], end.indices[1]]);
      }
      passage.passages = [
        {
          start: start_obj,
    ...
    bcv_passage.prototype.calculate_indices = function(match, adjust) {
      var end_index, indices, match_index, part, part_length, switch_type, _i, _len, _ref;
      switch_type = "book";
      indices = [];
      match_index = 0;
      adjust = parseInt(adjust, 10);
      _ref = match.split(/[\x1e\x1f]/);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        part = _ref[_i];
  ...
  bcv_parser.prototype.regexps.escaped_passage = /(?:^|[^\w\x1f\x1e])((?:(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:[\u2013\u2014\-]|through|thru|to)\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:\d+(?:th|nd|st)\s*ch(?:apter|a?pt\.?|a?p?\.?)?\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*))?\x1f(\d+)(?:\/[a-z])?\x1f(?:c(?:f|ompare|h(?:apters?|a?pts?|a?p?s?))|a(?:nd|lso)|\/p\x1f|[\d.:,;\x1e\x1f&\(\)\[\]\/"'\*=~\-\u2013\u2014\s]|[a-e](?!\w)|ff?\b|see|title(?![a-z])|thr(?:ough|u)|to|v(?:erses?|er|ss?|v)?|$)+)/g;
  bcv_parser.prototype.regexps.books_that_need_chapters = /^(?:Song?|Is|[12]The|Hebrew)$/i;
  bcv_parser.prototype.regexps.control = /[\x1e\x1f]/g;
  bcv_parser.prototype.regexps.first = "(?:1st|1|I|First)\\.?\\s*";
  bcv_parser.prototype.regexps.second = "(?:2nd|2|II|Second)\\.?\\s*";
  bcv_parser.prototype.regexps.third = "(?:3rd|3|III|Third)\\.?\\s*";
  ...
  bcv_parser.prototype.regexps.books = [
    {
      osis: ["Gen"],
      regexp: /\b(?:Ge(?:nn?[ei][ei]?s[eiu]s|nn?[es]is|nes[ei]|n)|G[en])(?:\b|(?=\d))/g
    }, {
      osis: ["Exod"],
      regexp: /\b(?:Ex(?:od[ui]s|od[se]|od|[do]?))(?:\b|(?=\d))/g
    }, {
      osis: ["Lev"],
      regexp: /\b(?:L(?:[ei]v[ei]t[ei]?cus|evi|ev|[ev]))(?:\b|(?=\d))/g
    }, {
      osis: ["Num"],
      regexp: /\b(?:N(?:umbers?|umb?|[um]))(?:\b|(?=\d))/g
    }, {
      osis: ["Deut"],
      regexp: /\b(?:D(?:eut[eo]?rono?my|ueteronomy|eut?|uet|t))(?:\b|(?=\d))/g
    }, {
      osis: ["Josh"],
      regexp: /\b(?:J(?:ou?sh?ua|o?sh|os))(?:\b|(?=\d))/g
    }, {
      osis: ["Judg"],
      regexp: /\b(?:J(?:udges|udg|d?gs?))(?:\b|(?=\d))/g
    }, {
      osis: ["Ruth"],
      regexp: /\b(?:R(?:uth?|th|u))(?:\b|(?=\d))/g
    }, {
      osis: ["Isa"],
      regexp: /\b(?:I(?:saiah|sais?ha?|s[ai]{2,}ha?|s[is]ah|sa[hi]?|sa?|a))(?:\b|(?=\d))/g
    }, {
      osis: ["2Sam"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "(?:Samu[ae]l[ls]?|Sam|Sma|S[am]?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Sam"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?Samu[ae]l[ls]?|" + bcv_parser.prototype.regexps.first + "(?:Sam|Sma|S[am]?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["2Kgs"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "Ki?n?g?s?)(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Kgs"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?K(?:i?ngs|in|gs)|" + bcv_parser.prototype.regexps.first + "Ki?n?g?s?)(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["2Chr"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "C(?:h?o?ron[io]cles?|hronicals|hro?n?|ron)|2\\s*Ch)(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Chr"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?C(?:h?o?ron[io]cles?|hronicals)|" + bcv_parser.prototype.regexps.first + "C(?:hro?n?|ron)|1\\s*Ch)(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Ezra"],
      regexp: /\b(?:E(?:zra?|sra))(?:\b|(?=\d))/g
    }, {
      osis: ["Neh"],
      regexp: /\b(?:N(?:eh[ei]m[ai]{1,3}h|eh?))(?:\b|(?=\d))/g
    }, {
      osis: ["Esth"],
      regexp: /\b(?:E(?:sth?er|sth?|s))(?:\b|(?=\d))/g
    }, {
      osis: ["Job"],
      regexp: /\b(?:Jo?b)(?:\b|(?=\d))/g
    }, {
      osis: ["Ps"],
      extra: "p",
      regexp: /\b(?:(?:(?:1[02-5]|[2-9])?(?:1\s*st|2\s*nd|3\s*rd))|1?1[123]\s*th|(?:150|1[0-4][04-9]|[1-9][04-9]|[4-9])\s*th)\s*Psalm\b/g
    }, {
      osis: ["Ps"],
      regexp: /\b(?:P(?:s[alm]{2,4}s?|a[slm]{3,4}s?|l[sam]{2,4}s?|s[as]?m?))(?:\b|(?=\d))/g
    }, {
      osis: ["Prov"],
      regexp: /\b(?:P(?:r[eo]?verbs?|robv?erbs|or?verbs|rovebs|ro?v?|rvbs?|v)|Oroverbs)(?:\b|(?=\d))/g
    }, {
      osis: ["Eccl"],
      regexp: /\b(?:E(?:cc?less?[ia]{1,4}s?tes?|cclesiastic?es|ccles|ccl?|cl?))(?:\b|(?=\d))/g
    }, {
      osis: ["Song"],
      regexp: /\b(?:(?:The\s*)?Songs?\s*of\s*(?:S[ao]lom[ao]ns?|Songs?)|(?:S(?:n?gs?|ongs?|\s*of\s*S|o?S|o[ln]?)))(?:\b|(?=\d))/g
    }, {
      osis: ["Jer"],
      regexp: /\b(?:Jer(?:emaiah|[ae]maih|[ae]miha|[aei]mi[ai]h|[ei]mi?ah|[ai]mih|[ae]mia|[am][im]ah|emi[he]?|e?)|J[er])(?:\b|(?=\d))/g
    }, {
      osis: ["Lam"],
      regexp: /\b(?:L(?:am[ei]ntations?|am?|m))(?:\b|(?=\d))/g
    }, {
      osis: ["Ezek"],
      regexp: /\b(?:E(?:[zx][ei]{1,2}ki?el|zekial|zek|z[ek]))(?:\b|(?=\d))/g
    }, {
      osis: ["Dan"],
      regexp: /\b(?:D(?:aniel|a?n|[al]))(?:\b|(?=\d))/g
    }, {
      osis: ["Hos"],
      regexp: /\b(?:H(?:osea|o?s|os?))(?:\b|(?=\d))/g
    }, {
      osis: ["Joel"],
      regexp: /\b(?:J(?:oel?|l))(?:\b|(?=\d))/g
    }, {
      osis: ["Amos"],
      regexp: /\b(?:Amo?s?)(?:\b|(?=\d))/g
    }, {
      osis: ["Obad"],
      regexp: /\b(?:O(?:badiah?|bidah|ba?d?))(?:\b|(?=\d))/g
    }, {
      osis: ["Jonah"],
      regexp: /\b(?:J(?:onah|on|nh))(?:\b|(?=\d))/g
    }, {
      osis: ["Mic"],
      regexp: /\b(?:M(?:ich?ah?|ic?))(?:\b|(?=\d))/g
    }, {
      osis: ["Nah"],
      regexp: /\b(?:N(?:ahum?|ah?))(?:\b|(?=\d))/g
    }, {
      osis: ["Hab"],
      regexp: /\b(?:H(?:abb?akk?[au]kk?|abk?))(?:\b|(?=\d))/g
    }, {
      osis: ["Zeph"],
      regexp: /\b(?:Z(?:ephana?iah?|e?ph?))(?:\b|(?=\d))/g
    }, {
      osis: ["Hag"],
      regexp: /\b(?:H(?:agg?ai|aggia[ih]|a?gg?))(?:\b|(?=\d))/g
    }, {
      osis: ["Zech"],
      regexp: /\b(?:Z(?:[ae]ch[ae]r[ai]{1,2}h|ach?|e?ch?))(?:\b|(?=\d))/g
    }, {
      osis: ["Mal"],
      regexp: /\b(?:M(?:alachi?|alichi|alaci|al))(?:\b|(?=\d))/g
    }, {
      osis: ["Matt"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.gospel + "M(?:at[th]{1,3}i?ew|atthwe|a?tt?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Mark"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.gospel + "M(?:a?rk?|k))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Luke"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.gospel + "L(?:uke?|[uk]))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1John"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.first + "J(?:o?phn|[ho][ho]n|onh|ohm|hn|o[hn]?|[hn]))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["2John"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "J(?:o?phn|[ho][ho]n|onh|ohm|hn|o[hn]?|[hn]))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["3John"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.third + "J(?:o?phn|[ho][ho]n|onh|ohm|hn|o[hn]?|[hn]))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["John"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.gospel + "J(?:o?phn|[ho][ho]n|onh|ohm|hn|oh|[hn]))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Acts"],
      regexp: /\b(?:A(?:cts\s*of\s*the\s*Apostles|cts*|ct?))(?:\b|(?=\d))/g
    }, {
      osis: ["Rom"],
      regexp: /\b(?:R(?:omans?|pmans|oamns|omands|omasn|om?s?|ms?))(?:\b|(?=\d))/g
    }, {
      osis: ["2Cor"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "C(?:h?orr?[in]{1,3}th[aio]{1,3}ns|orin[ai]?th[ai]{1,3}n[aio]{0,3}s|orinti[ao]ns|orinthian|orthians?|orint?h?|orth|or?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Cor"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?C(?:h?orr?[in]{1,3}th[aio]{1,3}ns|orin[ai]?th[ai]{1,3}n[aio]{0,3}s|orinti[ao]ns)|" + bcv_parser.prototype.regexps.first + "C(?:orinthian|orthians?|orint?h?|orth|or?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Gal"],
      regexp: /\b(?:G(?:alatians?|all?at[aino]{1,4}s|alat?|al?|l))(?:\b|(?=\d))/g
    }, {
      osis: ["Eph"],
      regexp: /\b(?:E(?:phesians?|phi?sians?|phesains?|sphesians|pehesians|h?pesians|phesiand|phesions|alat?|phe?s?|ph?|hp))(?:\b|(?=\d))/g
    }, {
      osis: ["Phil"],
      regexp: /\b(?:P(?:hil{1,}i?p{1,}[aei]{1,3}ns?|hi?li?p{0,2}|hil?|hp))(?:\b|(?=\d))/g
    }, {
      osis: ["Col"],
      regexp: /\b(?:C(?:[ao]ll?[ao]ss?i[ao]ns|olossi?ans?|ol?))(?:\b|(?=\d))/g
    }, {
      osis: ["2Thess"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "T(?:hess?[aeo]lon[ieaoc]{1,4}ns?|he?s{1,3}|h))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Thess"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?Thess?[aeo]lon[ieaoc]{1,4}ns?|" + bcv_parser.prototype.regexps.first + "T(?:he?s{1,3}|h))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["2Tim"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "T(?:imothy?|himoth?y|omothy|imoty|im?|m))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Tim"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?Timothy?|" + bcv_parser.prototype.regexps.first + "T(?:himoth?y|omothy|imoty|im?|m))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Titus"],
      regexp: /\b(?:T(?:itus|it?))(?:\b|(?=\d))/g
    }, {
      osis: ["Phlm"],
      regexp: /\b(?:Ph(?:ilemon|l?mn?|ilem?))(?:\b|(?=\d))/g
    }, {
      osis: ["Heb"],
      regexp: /\b(?:H(?:eb[rew]{1,3}s|[ew]{0,2}brew{1,2}s|ebrew|eb))(?:\b|(?=\d))/g
    }, {
      osis: ["Jas"],
      regexp: /\b(?:J(?:ames?|a[ms]?|ms?))(?:\b|(?=\d))/g
    }, {
      osis: ["2Pet"],
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "P(?:eter?|e?t?r?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["1Pet"],
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?Peter|" + bcv_parser.prototype.regexps.first + "P(?:eter?|e?t?r?))(?:\\b|(?=\\d))", "g")
    }, {
      osis: ["Jude"],
      regexp: /\b(?:Ju?de)(?:\b|(?=\d))/g
    }, {
      osis: ["Rev"],
      regexp: /\b(?:R(?:ev[aeo]?lations?|evel|e?v|e))(?:\b|(?=\d))/g
    }, {
      osis: ["Ezek", "Ezra"],
      regexp: /\bEz(?:\b|(?=\d))/g
    }, {
  ...
  bcv_parser.prototype.regexps.apocrypha = [
    {
      osis: ["Tob"],
      apocrypha: true,
      regexp: /\b(?:Tobi?t?)(?:\b|(?=d))/g
    }, {
      osis: ["Jdt"],
      apocrypha: true,
      regexp: /\b(?:Ju?di?th?)(?:\b|(?=d))/g
    }, {
      osis: ["GkEsth"],
      apocrypha: true,
      regexp: /\b(?:Greek\s*Esther|Esther\s*\(Greek\)|G(?:ree)?k\s*Esth?)(?:\b|(?=d))/g
    }, {
      osis: ["Wis"],
      apocrypha: true,
      regexp: /\b(?:(?:The\s*)Wisd?(?:om)?\s*of\s*Solomon|Wisdom|Wisd?)(?:\b|(?=d))/g
    }, {
      osis: ["Sir"],
      apocrypha: true,
      regexp: /\b(?:Sirach|Sir|Eccl[eu]siasticus|Ecclus|Eccs|(?:The\s*)Wisdom\s*of\s*Jesus\s*(?:Son\s*of|ben)\s*Sirach)(?:\b|(?=d))/g
    }, {
      osis: ["Bar"],
      apocrypha: true,
      regexp: /\b(?:Baruch|Bar)(?:\b|(?=d))/g
    }, {
      osis: ["PrAzar"],
      apocrypha: true,
      regexp: /\b(?:(?:The\s*)?Pr(?:ayers?)?\s*of\s*Azariah?|Azariah?|PrAza?r)(?:\b|(?=d))/g
    }, {
      osis: ["Sus"],
      apocrypha: true,
      regexp: /\b(?:Susannah?|Shoshana|Sus)(?:\b|(?=d))/g
    }, {
      osis: ["Bel"],
      apocrypha: true,
      regexp: /\b(?:Bel\s*(?:and|&)\s*(?:the\s*)?(?:Dragon|Serpent|Snake)|Bel)(?:\b|(?=d))/g
    }, {
      osis: ["SgThree"],
      apocrypha: true,
      regexp: /\b(?:(?:The\s*)?Song\s*of\s*(?:the\s*)?(?:3|Three)\s*(?:Holy\s*Children|Young\s*Men|Youths|Jews)|S\.?\s*(?:of)?\s*(?:Three|Th|3)\.?\s*(?:Ch|Y)|So?n?gThree)(?:\b|(?=d))/g
    }, {
      osis: ["EpJer"],
      apocrypha: true,
      regexp: /\b(?:(?:The\s*)?(?:Ep(?:istle)?|Let(?:ter))\.?\s*of\s*Jeremiah|EpJer)(?:\b|(?=d))/g
    }, {
      osis: ["1Macc"],
      apocrypha: true,
      regexp: RegExp("\\b(?:(?:" + bcv_parser.prototype.regexps.first + ")?Mac{1,3}ab{1,3}e{1,3}s?|" + bcv_parser.prototype.regexps.first + "Mac{1,3}|1\\s*Mc)(?:\\b|(?=d))", "g")
    }, {
      osis: ["2Macc"],
      apocrypha: true,
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "Mac{1,3}(?:ab{1,3}e{1,3}s?)?|2\\s*Mc)(?:\\b|(?=d))", "g")
    }, {
      osis: ["3Macc"],
      apocrypha: true,
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.third + "Mac{1,3}(?:ab{1,3}e{1,3}s?)?|3\\s*Mc)(?:\\b|(?=d))", "g")
    }, {
      osis: ["4Macc"],
      apocrypha: true,
      regexp: /\b(?:(?:4th|4|IV|Fourth)\.?\s*Mac{1,3}(?:ab{1,3}e{1,3}s?)?|4\s*Mc)(?:\b|(?=d))/g
    }, {
      osis: ["1Esd"],
      apocrypha: true,
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.first + "Esdras|1\\s*Esdr?)(?:\\b|(?=d))", "g")
    }, {
      osis: ["2Esd"],
      apocrypha: true,
      regexp: RegExp("\\b(?:" + bcv_parser.prototype.regexps.second + "Esdras|2\\s*Esdr?)(?:\\b|(?=d))", "g")
    }, {
      osis: ["PrMan"],
      apocrypha: true,
      regexp: /\b(?:(?:(?:The\s*)Pr(?:ayers?)?\s*(?:of\s*)?M[ae]n{1,2}[ae]s{1,2}[ae]h)|PrMan)(?:\b|(?=d))/g
    }
  ];
  ...
  var result = {
    ...
    parse: function(input, startRule) {
      ...
      function parse_sequence() {
        ...
        if (result3 !== null) {
          var result52 = parse_cb_range();
          if (result52 !== null) {
            var result4 = result52;
          } else {
            var result51 = parse_bcv_hyphen_range();
            if (result51 !== null) {
              var result4 = result51;
            } else {
              var result50 = parse_range();
              if (result50 !== null) {
                var result4 = result50;
              } else {
                var result49 = parse_ff();
                if (result49 !== null) {
                  var result4 = result49;
                } else {
                  var result48 = parse_bcv_comma();
                  if (result48 !== null) {
                    var result4 = result48;
                  } else {
                    var result47 = parse_bc_title();
                    if (result47 !== null) {
                      var result4 = result47;
                    } else {
                      var result46 = parse_bcv();
                      if (result46 !== null) {
                        var result4 = result46;
                      } else {
                        var result45 = parse_bcv_weak();
                        if (result45 !== null) {
                          var result4 = result45;
                        } else {
                          var result44 = parse_bc();
                          if (result44 !== null) {
                            var result4 = result44;
                          } else {
                            var result43 = parse_cv_psalm();
                            if (result43 !== null) {
                              var result4 = result43;
                            } else {
                              var result42 = parse_bv();
                              if (result42 !== null) {
                                var result4 = result42;
                              } else {
                                var result41 = parse_b_range();
                                if (result41 !== null) {
                                  var result4 = result41;
                                } else {
                                  var result40 = parse_c_psalm();
                                  if (result40 !== null) {
                                    var result4 = result40;
                                  } else {
                                    var result39 = parse_b();
                                    if (result39 !== null) {
                                      var result4 = result39;
                                    } else {
                                      var result38 = parse_cbv();
                                      if (result38 !== null) {
                                        var result4 = result38;
                                      } else {
                                        var result37 = parse_cbv_ordinal();
                                        if (result37 !== null) {
                                          var result4 = result37;
                                        } else {
                                          var result36 = parse_cb();
                                          if (result36 !== null) {
                                            var result4 = result36;
                                          } else {
                                            var result35 = parse_cb_ordinal();
                                            if (result35 !== null) {
