...
var bcv_regexps = class {
  constructor() {
    ...
    this.first = String.raw`(?:முதலாவது|Mutalāvatu|1)\.?\s*`;
    this.second = String.raw`(?:இரண்டாம|இரண்டாவது|Iraṇṭāvatu|2)\.?\s*`;
    this.third = String.raw`(?:முன்றாம்|மூன்றாவது|Mūṉṛāvatu|3)\.?\s*`;
    this.range_and = String.raw`(?:[&\u2013\u2014-]|மற்றும்|to)`;
    ...
    this.all_books = [
      ...
      },
      {
        osis: ["1John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Yōvaṉ\s*Eḻutiya\s*Mutalāvatu\s*Nirupam|யோவ(?:ான்\s*(?:எழுதிய\s*முதல(?:்\s*திருமுக|ாம்\s*கடித)|முதல்\s*திருமுக)|ன்\s*எழுதிய\s*முதலாவது\s*நிருப)ம்|(?:(?:Mutalāvatu|1\.)\s*யோவான|முதலாவது\s*யோவான|1\s*(?:அருளப்பர|யோவான))்|1(?:\s*Yōvaṉ|John)|1\s*யோ(?:வா)?))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2John"],
        testament: "n",
      ...
      },
      {
        osis: ["3John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:யோவ(?:ான்\s*(?:எழுதிய\s*ம(?:ுன்றாம்\s*திருமுக|ூன்றாம்\s*கடித)|மூன்றாம்\s*திருமுக)|ன்\s*எழுதிய\s*மூன்றாவது\s*நிருப)ம்|Yōvaṉ\s*Eḻutiya\s*Mūṉṛāvatu\s*Nirupam|(?:(?:Mūṉṛāvatu|3\.)\s*யோவான|ம(?:ூன்றாவது|ுன்றாம்)\s*யோவான|3\s*(?:அருளப்பர|யோவான))்|3(?:\s*Yōvaṉ|John)|3\s*யோ(?:வா)?))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}]))((?:யோவான்\s*எழுதிய\s*சுவிசேஷம்|யோவான்\s*(?:எழுதிய\s*)?நற்செய்தி|அருளப்பர்\s*நற்செய்தி|Yōvāṉ\s*Naṛceyti|யோவா(?:ன்)?|Yōvāṉ|John))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Acts"],
        testament: "n",
...
function peg$parse(input, options) {
  ...
  var peg$e19 = peg$literalExpectation("அதி", true);
  var peg$e20 = peg$literalExpectation("காரம்", true);
  ...
  var peg$e25 = peg$literalExpectation("மற்றும்", true);
