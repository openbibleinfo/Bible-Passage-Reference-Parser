...
var bcv_regexps = class {
  constructor() {
    ...
    // The `t[íi]tulo` has `[a-z]` instead of `\w` because it could be followed by a number.
    // [a-d] allows `1:1a`.
    this.escaped_passage = /(?:^|[^\x1e\x1f\p{L}\p{N}])((?:(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:[\u2013\u2014\-]|through|thru|to)\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:\d+(?:th|nd|st)\s*ch(?:apter|a?pt\.?|a?p?\.?)?\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*))?\x1f(\d+)(?:\/\d+)?\x1f(?:\/\d+\x1f|[\d\s.:,;\x1e\x1f&\(\)（）\[\]\\/"'\*=~\-–—]|t[íi]tulo(?![a-z])|vers[íi]culos|cap[íi]tulos|vers[íi]culo|cap[íi]tulo|e(?!\s+sig)|e\s+sig|caps|vers|cap|ver|vss|vs|vv|á|v|[a-d](?!\w)|$)+)/giu;
    ...
    this.match_end_split = /\d\W*t[íi]tulo|\d\W*e\s+sig(?:[\s*]*\.)?|\d[\s*]*[a-d](?!\w)|\x1e(?:[\s*]*[)\]\uff09])?|[\d\x1f]/gi;
    this.control = /[\x1e\x1f]/g;
    // These are needed for ranges outside of this class.
    this.first = String.raw`(?:Primeir[ao]|1[ao]?|I)\.?\s*`;
    this.second = String.raw`(?:Segund[ao]|2[ao]?|II)\.?\s*`;
    this.third = String.raw`(?:Terceir[ao]|3[ao]?|III)\.?\s*`;
    this.range_and = String.raw`(?:[&\u2013\u2014-]|e(?!\s+sig)|á)`;
    ...
    this.all_books = [
      ...
      },
      {
        osis: ["2Esd"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Esdras|(?:(?:2[ao]|2)|II)\.\s*Esdras|(?:2[ao]|II)\s*Esdras|2(?:\s*Esdras|(?:\s*Esd?|Esd))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Esd"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Esdras|(?:(?:1[ao]|I)|1)\.\s*Esdras|(?:1[ao]|I)\s*Esdras|1(?:\s*Esdras|(?:\s*Esd?|Esd))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Isa"],
        testament: "o",
      ...
      },
      {
        osis: ["2Sam"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*(?:Reinos|Samuel)|(?:(?:2[ao]|2)|II)\.\s*(?:Reinos|Samuel)|(?:2[ao]|II)\s*(?:Reinos|Samuel)|2(?:(?:(?:\s*Reinos|(?:\s*S|Sa)m)|\s*S(?:am?)?)|\s*Samuel)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Sam"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*(?:Reinos|Samuel)|(?:(?:1[ao]|I)|1)\.\s*(?:Reinos|Samuel)|(?:1[ao]|I)\s*(?:Reinos|Samuel)|1(?:(?:(?:\s*Reinos|(?:\s*S|Sa)m)|\s*S(?:am?)?)|\s*Samuel)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2Kgs"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Quart[ao]\s*Reinos|(?:Segund[ao]\s*Rei|(?:(?:2[ao]|2)|II)\.\s*Rei|(?:2[ao]|II)\s*Rei|2(?:\s*Rei|Kg))s|(?:IV\.|4(?:[ao])?\.)\s*Reinos|(?:IV\s*|4(?:[ao])?\s*)Reinos|2\s*R(?:e?|s)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Kgs"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Terceir[ao]\s*Reinos|(?:Primeir[ao]\s*Rei|(?:1(?:[ao])?\.|I\.)\s*Rei|(?:1[ao]\s*|I\s*)Rei|1(?:\s*Rei|Kg))s|(?:III\.|3(?:[ao])?\.)\s*Reinos|(?:III\s*|3(?:[ao])?\s*)Reinos|1\s*R(?:e?|s)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2Chr"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Cr[oô]nicas|(?:(?:2[ao]|2)|II)\.\s*Cr[oô]nicas|(?:2[ao]|II)\s*Cr[oô]nicas|2\s*Cr[oô]nicas|2(?:\s*Cr(?:[oô]n?)?|Chr)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Chr"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Cr[oô]nicas|(?:(?:1[ao]|I)|1)\.\s*Cr[oô]nicas|(?:1[ao]|I)\s*Cr[oô]nicas|1\s*Cr[oô]nicas|1(?:\s*Cr(?:[oô]n?)?|Chr)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Ezra"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}]))(E(?:sdras|sdr?|zra|d))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Neh"],
        testament: "o",
      ...
      },
      {
        osis: ["Job"],
        testament: "o",
        regexp: /(?:^|(?<=[^\p{L}]))(J(?:ob|ó))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["PrAzar"],
        testament: "a",
      ...
      },
      {
        osis: ["SgThree"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:C(?:[aâ]ntico\s*dos\s*(?:Tr[eê]s\s*Jovens|3\s*Jovens)|3J)|SgThree))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Song"],
        testament: "o",
      ...
      },
      {
        osis: ["1Macc"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Macabeus|(?:(?:1[ao]|I)|1)\.\s*Macabeus|(?:1[ao]|I)\s*Macabeus|1(?:\s*Macabeus|(?:\s*M(?:ac)?|(?:Mac|\s*M)c))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2Macc"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Macabeus|(?:(?:2[ao]|2)|II)\.\s*Macabeus|(?:2[ao]|II)\s*Macabeus|2(?:\s*Macabeus|(?:\s*M(?:ac)?|(?:Mac|\s*M)c))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["3Macc"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Terceir[ao]\s*Macabeus|(?:III|(?:3[ao]|3))\.\s*Macabeus|(?:III|3[ao])\s*Macabeus|3(?:\s*Macabeus|(?:\s*M(?:ac)?|(?:Mac|\s*M)c))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["4Macc"],
        testament: "a",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Quart[ao]\s*Macabeus|(?:(?:4[ao]|4)|IV)\.\s*Macabeus|(?:4[ao]|IV)\s*Macabeus|4(?:\s*Macabeus|(?:\s*M(?:ac)?|(?:Mac|\s*M)c))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Mark"],
        testament: "n",
      ...
      },
      {
        osis: ["1John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Jo[aã]o|(?:(?:1[ao]|I)|1)\.\s*Jo[aã]o|(?:1[ao]|I)\s*Jo[aã]o|1\s*Jo[aã]o|1(?:John|\s*Jo?)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Jo[aã]o|(?:(?:2[ao]|2)|II)\.\s*Jo[aã]o|(?:2[ao]|II)\s*Jo[aã]o|2\s*Jo[aã]o|2(?:John|\s*Jo?)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["3John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Terceir[ao]\s*Jo[aã]o|(?:III|(?:3[ao]|3))\.\s*Jo[aã]o|(?:III|3[ao])\s*Jo[aã]o|3\s*Jo[aã]o|3(?:John|\s*Jo?)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["John"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}]))(Jo(?:hn|[aã]o)?)(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Acts"],
        testament: "n",
      ...
      },
      {
        osis: ["2Cor"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Cor[ií]ntios|(?:(?:2[ao]|2)|II)\.\s*Cor[ií]ntios|(?:2[ao]|II)\s*Cor[ií]ntios|2\s*Cor[ií]ntios|2(?:\s*Cor?|Cor)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Cor"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Cor[ií]ntios|(?:(?:1[ao]|I)|1)\.\s*Cor[ií]ntios|(?:1[ao]|I)\s*Cor[ií]ntios|1\s*Cor[ií]ntios|1(?:\s*Cor?|Cor)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Gal"],
        testament: "n",
      ...
      },
      {
        osis: ["2Thess"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Tessalonicenses|(?:(?:(?:2[ao]|2)|II)\.\s*Tessalonicense|(?:(?:2[ao]|II)\s*Tessalonicense|(?:2\s*Tessalonicense|(?:2\s*T(?:es)?|2Thes))))s))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Thess"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Tessalonicenses|(?:(?:(?:1[ao]|I)|1)\.\s*Tessalonicense|(?:(?:1[ao]|I)\s*Tessalonicense|(?:1\s*Tessalonicense|(?:1\s*T(?:es)?|1Thes))))s))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["2Tim"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Tim[oó]teo|(?:(?:2[ao]|2)|II)\.\s*Tim[oó]teo|(?:2[ao]|II)\s*Tim[oó]teo|2\s*Tim[oó]teo|2(?:\s*Tim?|(?:\s*T|Ti)m)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Tim"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Tim[oó]teo|(?:(?:1[ao]|I)|1)\.\s*Tim[oó]teo|(?:1[ao]|I)\s*Tim[oó]teo|1\s*Tim[oó]teo|1(?:\s*Tim?|(?:\s*T|Ti)m)))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Titus"],
        testament: "n",
      ...
      },
      {
        osis: ["2Pet"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Segund[ao]\s*Pedro|(?:(?:2[ao]|2)|II)\.\s*Pedro|(?:2[ao]|II)\s*Pedro|2(?:\s*Pedro|(?:\s*Ped?|Pet))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["1Pet"],
        testament: "n",
        regexp: /(?:^|(?<=[^\p{L}\p{N}])(?<!\d:(?=\d)))((?:Primeir[ao]\s*Pedro|(?:(?:1[ao]|I)|1)\.\s*Pedro|(?:1[ao]|I)\s*Pedro|1(?:\s*Pedro|(?:\s*Ped?|Pet))))(?:(?=[\d\s.:,;\x1e\x1f&\(\)（）\[\]\/"'\*=~\-–—])|$)/giu
      },
      {
        osis: ["Jude"],
        testament: "n",
...
function peg$parse(input, options) {
  ...
  var peg$e13 = peg$literalExpectation("sig", false);
  ...
  var peg$e20 = peg$literalExpectation("cap", false);
  var peg$e21 = peg$classExpectation(["í", "i"], false, true);
  var peg$e22 = peg$literalExpectation("tulos", false);
  var peg$e23 = peg$literalExpectation("tulo", false);
  var peg$e24 = peg$literalExpectation("s", false);
  var peg$e25 = peg$literalExpectation("v", false);
  var peg$e26 = peg$literalExpectation("ers", false);
  var peg$e27 = peg$literalExpectation("culos", false);
  var peg$e28 = peg$literalExpectation("culo", false);
  var peg$e29 = peg$literalExpectation("er", false);
  var peg$e30 = peg$literalExpectation("ss", false);
  ...
  var peg$e35 = peg$literalExpectation("á", true);
  ...
  var peg$e40 = peg$literalExpectation("the", false);
  var peg$e41 = peg$literalExpectation("book", false);
  var peg$e42 = peg$classExpectation(["(", "["], false, false);
  var peg$e43 = peg$classExpectation([")", "]"], false, false);
  var peg$e44 = peg$literalExpectation("", false);
  var peg$e45 = peg$literalExpectation(",000", false);
