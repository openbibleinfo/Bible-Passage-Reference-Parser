...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / cv / cv_weak / integer / c / v) sp "et" space "suivant" abbrev? ![a-z]
    { return {"type": "ff", "value": [val_1], "indices": [offset(), peg$currPos - 1]} }

integer_title
  = val_1:integer (cv_sep / sequence_sep)? "titre"
...
/* BCV helpers */
c_explicit
  = sp ( "chapitres" / "chapitre" / "chap" abbrev? / "chs" abbrev? / "ch" abbrev? ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "versets" / "verset" / "vers" abbrev? / "ver" abbrev? / "vv" abbrev? / "v" abbrev? ) ![a-z] sp
