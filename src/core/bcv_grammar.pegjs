...
/* Multiples */
sequence
  = val_1:(cb_range / bcv_hyphen_range / range / next_v / ff / bcv_comma / bc_title / ps151_bcv / bcv / bcv_weak / ps151_bc / bc / cv_psalm / bv / c_psalm / b / cbv / cbv_ordinal / cb / cb_ordinal / context) val_2:(sep_val:(c_sep / sequence_sep?) seq_post:sequence_post { if (sep_val && sep_val.type && sep_val.type === "c_explicit") seq_post.explicit_context = "c"; return [seq_post] })+
    { val_2.unshift([val_1]); const r = range(); return {type: "sequence", value: val_2, indices: [r.start, r.end - 1]} }

sequence_post_enclosed
  = "(" sp sep_val_1:(c_sep / sequence_sep?) val_1:(sequence_post) val_2:(sep_val:(c_sep / sequence_sep?) seq_post:sequence_post { if (sep_val && sep_val.type && sep_val.type === "c_explicit") seq_post.explicit_context = "c"; return [seq_post] })* sp ")"
    { if (typeof(val_2) === "undefined") val_2 = []; if (sep_val_1 && sep_val_1.type && sep_val_1.type === "c_explicit") val_1.explicit_context = "c"; val_2.unshift([val_1]); const r = range(); return {type: "sequence_post_enclosed", value: val_2, indices: [r.start, r.end - 1]} }
...
range
  = val_1:(bcv_comma / bc_title / ps151_bcv / bcv / bcv_weak / ps151_bc / bc / cv_psalm / bv / b &(range_sep (bcv_comma / bc_title / ps151_bcv / bcv / bcv_weak / ps151_bc / bc / bv / b)) / cbv / cbv_ordinal / c_psalm / cb / cb_ordinal / c_title / integer_title / cv / cv_weak / integer / c / v) range_sep val_2:(next_v / ff / bcv_comma / bc_title / ps151_bcv / bcv / bcv_weak / ps151_bc / bc / cv_psalm / bv / b / cbv / cbv_ordinal / c_psalm / cb / cb_ordinal / c_title / integer_title / cv / integer / cv_weak / c / v)
    { if (val_1.length && val_1.length === 2) val_1 = val_1[0]; // for `b`, which returns [object, undefined]
      const r = range(); return {type: "range", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

/* Singles */
b
  = "\x1f" val:any_integer ("/" [1-8])? "\x1f"
    { const r = range(); return {type: "b", value: val.value, indices: [r.start, r.end - 1]} }
...
bc
  = val_1:b (v_explicit &cv / cv_sep / cv_sep_weak / range_sep / sp) val_2:c
    { const r = range(); return {type: "bc", value: [val_1, val_2], indices: [r.start, r.end - 1]} }
...
bc_title
  = val_1:(ps151_bc / bc) &{ return val_1.value[1].value[0].partial == null; } val_2:title
    { const r = range(); return {type: "bc_title", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

bcv
  = val_1:(ps151_bc / bc) !("." v_explicit v / sequence_sep? v_explicit cv) ((cv_sep / sequence_sep)? v_explicit / cv_sep) val_2:v
    { const r = range(); return {type: "bcv", value: [val_1, val_2], indices: [r.start, r.end - 1]} }
...
bcv_hyphen_range
  = val_1:b ("-" / space)? val_2:c &{ return val_2.value[0].partial == null; } "-" val_3:v "-" val_4:v
    { const r = range(); return {type: "range", value: [{type: "bcv", value: [{type: "bc", value: [val_1, val_2], indices: [val_1.indices[0], val_2.indices[1]]}, val_3], indices: [val_1.indices[0], val_3.indices[1]]}, val_4], indices: [r.start, r.end - 1]} }
...
bv
  = val_1:b (cv_sep / cv_sep_weak / range_sep / sequence_sep &v_explicit / sp) val_2:v
    { const r = range(); return {type: "bv", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

c
  = c_explicit? val:integer
    { const r = range(); return {type: "c", value: [val], indices: [r.start, r.end - 1]} }

// The &{} disallows letters in chapters.
cb
  = c_explicit val_1:c &{ return val_1.value[0].partial == null; } in_book_of? val_2:b
    { const r = range(); return {type: "bc", value: [val_2, val_1], indices: [r.start, r.end - 1]} }

// The &{} disallows letters in chapters.
cb_range
  = c_explicit val_1:c &{ return val_1.value[0].partial == null; } range_sep val_2:c in_book_of? val_3:b
    { const r = range(); return {type: "cb_range", value: [val_3, val_1, val_2], indices: [r.start, r.end - 1]} }
...
c_psalm
  = "\x1f" val:any_integer "/1\x1f"
    { const r = range(); return {type: "c_psalm", value: val.value, indices: [r.start, r.end - 1]} }

cv_psalm
  = val_1:c_psalm sequence_sep? v_explicit val_2:v
    { const r = range(); return {type: "cv_psalm", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

// The &{} disallows letters in chapters.
c_title
  = c_explicit val_1:c &{ return val_1.value[0].partial == null; } val_2:title
    { const r = range(); return {type: "c_title", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

// The &{} disallows letters in chapters.
cv
  = v_explicit? val_1:c &{ return val_1.value[0].partial == null; } !("." v_explicit v) (cv_sep? v_explicit / cv_sep) val_2:v
    { const r = range(); return {type: "cv", value: [val_1, val_2], indices: [r.start, r.end - 1]} }
...
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp ff_value
    { const r = range(); return {type: "ff", value: [val_1], indices: [r.start, r.end - 1]} }
...
next_v
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp next_value
    { const r = range(); return {type: "next_v", value: [val_1], indices: [r.start, r.end - 1]} }
...
integer_title
  = val_1:integer val_2:title
    { const r = range(); return {type: "integer_title", value: [val_1, val_2], indices: [r.start, r.end - 1]} }

context
  = "\x1f" val:any_integer "/9\x1f"
    { const r = range(); return {type: "context", value: val.value, indices: [r.start, r.end - 1]} }
...
ps151_bc
  = val:ps151_b ".1" ![0-9]
    { const r = range(); return {type: "bc", value: [val, {type: "c", value: [{type: "integer", value: 151, indices: [r.end - 2, r.end - 1]}], indices: [r.end - 2, r.end - 1]}], indices: [r.start, r.end - 1]} }

ps151_bcv
  = val_1:ps151_bc "." val_2:integer
    { const r = range(); return {type: "bcv", value: [val_1, {type: "v", value: [val_2], indices: [val_2.indices[0], val_2.indices[1]]}], indices: [r.start, r.end - 1]} }
...
v
  = v_explicit? val:integer
    { const r = range(); return {type: "v", value: [val], indices: [r.start, r.end - 1]} }
...
title
  = (cv_sep / sequence_sep)? val:title_value
    { const r = range(); return {type:"title", value: [val], indices: [r.start, r.end - 1]} }
...
translation_sequence_enclosed
  = sp [\(\[] sp val:(translation (sequence_sep translation)*) sp [\)\]]
    { const r = range(); return {type: "translation_sequence", value: val, indices: [r.start, r.end - 1]} }
...
translation
  = "\x1e" val:any_integer "\x1e"
    { const r = range(); return {type: "translation", value: val.value, indices: [r.start, r.end - 1]} }

/* Base nodes */
integer
  = val_1:integer_value val_2:( sp ab )?
    { const r = range(); return {type: "integer", value: parseInt(val_1.join(""), 10), partial: (val_2 != null) ? val_2[1].join("") : null, indices: [r.start, r.end - 1]} }
...
any_integer
  = val:([0-9]+)
    { const r = range(); return {type: "integer", value: parseInt(val.join(""), 10), indices: [r.start, r.end - 1]} }

word
  = val:( [^\x1f\x1e\(\[]+ )
    { const r = range(); return {type: "word", value: val.join(""), indices: [r.start, r.end - 1]} }
...
word_parenthesis
  = val:[\(\[]
    { const r = range(); return {type: "stop", value: val, indices: [r.start, r.end - 1]} }
