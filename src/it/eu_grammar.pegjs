...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / cv / cv_weak / integer / c / v) sp ( "ss" / "," space "ecc" / "ecc" ) abbrev? ![a-z]
    { return {"type": "ff", "value": [val_1], "indices": [offset(), peg$currPos - 1]} }

integer_title
  = val_1:integer (cv_sep / sequence_sep)? "titolo"
...
/* BCV helpers */
c_explicit
  = sp ( "capitoli" / "capitolo" / "capp" / "cap" / "cc" ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "versetto" / "versetti" / "versi" / "vv" / "v" ) ![a-z] sp
