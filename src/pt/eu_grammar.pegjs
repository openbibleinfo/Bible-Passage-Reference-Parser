...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / cv / cv_weak / integer / c / v) sp "e" space "sig" abbrev? ![a-z]
...
v_letter
  = v_explicit? val:integer sp !( "e" space "sig" ) [a-d] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "cap" [íi]i "tulos" / "cap" [íi]i "tulo" / "caps" / "cap" ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "vers" [íi]i "culos" / "vers" [íi]i "culo" / "vers" abbrev? / "ver" abbrev? / "vss" abbrev? / "vs" abbrev? / "vv" abbrev? / "v" abbrev? ) ![a-z] sp
