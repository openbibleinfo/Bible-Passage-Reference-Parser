...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp ( "nn" / "n" ) abbrev? ![a-z]
    { return {"type": "ff", "value": [val_1], "indices": [offset(), peg$currPos - 1]} }

integer_title
  = val_1:integer (cv_sep / sequence_sep)? "tytuł"i
...
v_letter
  = v_explicit? val:integer sp !( ( "nn" / "n" ) ) [a-e] ![a-z]
...
/* BCV helpers */
c_explicit
  = sp ( "rozdziały"i / "rozdział"i [óo]i "w" / "rozdział"i / "rozdz" / "rr" abbrev? / "r" abbrev? ) sp
    { return {"type": "c_explicit"} }

v_explicit
  = sp ( "werset" [óo]i "w" / "wersety" / "werset" / "wers" abbrev? / "w" abbrev? ) ![a-z] sp
