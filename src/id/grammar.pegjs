...
// No `b` or `ps151`.
ff
  = val_1:(bcv / bcv_weak / bc / bv / cv / cv_weak / integer / c / v) sp "dst" abbrev? ![a-z]
...
v_letter
  = v_explicit? val_1:integer sp !( "dst" ) val_2:( [a-e] ) ![a-z]
    { var r = range(); return {"type": "v", "value": [val_1], "letter": val_2, "indices": [r.start, r.end - 1]} }
...
sequence_sep
  = ([,;/:&\-\u2013\u2014~] / "." !(sp "." sp ".") / "bandingkan" / "lih" abbrev? / "bdk" abbrev? / space)+
    { return "" }
